 ...
 * Usage:
 *
 * 1. use static method getInstance() to create a CptComposer instance.
 * 2. setup Ecclesiastical mode with setMode() for CptCantusFirmus if needed.
 * 3. setup task with setTask();
 * 4. setup logger with setLogger();
 * 5. initialize graphs and population with init_graphs() and init_population().
 * 6. call navigate() and evaporate() to generate music threads.
 * 7. call isAimAchieved() to test if all threads are completed.
 * </pre>
 *
