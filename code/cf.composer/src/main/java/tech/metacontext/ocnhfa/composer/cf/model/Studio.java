...
import tech.metacontext.ocnhfa.composer.cf.utils.selectors.StyleSelector;
...
public class Studio {
  ...
  public void run() {
    ...
    // Print StyleSelector test report (to be removed)
    this.qualified_threads.entrySet().stream()
            .sorted(Entry.comparingByKey(Comparator.comparing(Composer::getId)))
            .peek(entry -> System.out.printf("Composer#%s: ", entry.getKey().getId()))
            .map(Entry::getValue)
            .map(StyleSelector::totalReport)
  ...
  public void saveScore(Clef clef) {
    ...
    this.qualified_threads.entrySet().stream().forEach(e -> {
      ...
      cf_path.mkdirs();
      //style selector: for testing purpose, to be removed
      var selector = Objects.isNull(this.style)
              ? Integer.valueOf(id.substring(id.length() - 1))
              : this.style;
      System.out.println("style selector=" + selector);
      //applying filter
      var selected_pool = e.getValue().stream()
              .sorted()
              //applying filters
              //style selector: for testing purpose, to be removed
              .filter(StyleSelector.get(selector))
              .limit(target_size)
              .collect(Collectors.toList());
      IntStream.range(0, selected_pool.size())
              //.peek(i -> System.out.printf("%2d.%s\n", i + 1, selected_pool.get(i)))
              .forEach(i -> {
      ...
      var score = new Score(id, this.model);
  ...
  private void preset_composers() {
    ...
    var path = new File(PROJECT_DIR, this.model);
  ...
  public Studio setModel(String model, String folder, Integer style) {
